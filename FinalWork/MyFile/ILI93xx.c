	...
	{	
		if((*s) < 128) 
		{
			...
			else 
			{
				...
				for(j=0;j<8;j++) 
					{
				    	if(asc16[k*16+i]&(0x80>>j))	LCD_DrawPoint(x+j,y+i);
//						else 
//						{
//							if (fc!=bc) 
			...
			for (k=0;k<hz16_num;k++) 
			{
			  ...
			  { 
				    for(i=0;i<16;i++)
				    {
						for(j=0;j<8;j++) 
							{
						    	if(hz16[k].Msk[i*2]&(0x80>>j))	LCD_DrawPoint(x+j,y+i);
//								else {
//									if (fc!=bc) 
//										//Gui_DrawPoint(x+j,y+i,bc);
//								}
						...
						for(j=0;j<8;j++) 
							{
						    	if(hz16[k].Msk[i*2+1]&(0x80>>j))	LCD_DrawPoint(x+j+8,y+i);
//								else 
//								{
//									if (fc!=bc) 
...
int32_t dx,         // difference in x's
	...
	dx2 = dx << 1;
	dy2 = dy << 1;
	//x戮脿脌毛麓贸脫脷y戮脿脌毛拢卢脛脟脙麓脙驴赂枚x脰谩脡脧脰禄脫脨脪禄赂枚碌茫拢卢脙驴赂枚y脰谩脡脧脫脨脠么赂脡赂枚碌茫
	//脟脪脧脽碌脛碌茫脢媒碌脠脫脷x戮脿脌毛拢卢脪脭x脰谩碌脻脭枚禄颅碌茫
	if (dx > dy)
	{
		...
		// draw the line
		for (index=0; index <= dx; index++)//脪陋禄颅碌脛碌茫脢媒虏禄禄谩鲁卢鹿媒x戮脿脌毛
		{
			//禄颅碌茫
			LCD_DrawPoint(x0,y0);
